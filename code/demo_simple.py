...
def build_system_prompt_simple(user_prompt: str, ppa_goal: str | None) -> Path:
    general_prompt_instruction = textwrap.dedent("""
    You are an expert chip architect.
    Your task is to analyze the given Design Prompt
    to generate a hierarchical hardware design structure where all the submodule will eventually build the topmodule.                                        
    """)

    gerenal_prompt_template = """{
    "template_name": "hierarchical_mux64_verilog",
    "version": "1.0.0",
    "notes": "General prompt template for generating a hierarchical 64:1 MUX in synthesizable SystemVerilog/Verilog. Edit placeholders in `{{ ... }}` before use.",
    "messages": [
        {
        "role": "system",
        "content": "You are an expert hardware design assistant. Produce synthesizable, lint-clean, timing-friendly RTL and (optionally) a minimal self-checking testbench.\nFollow these rules strictly:\n1) Target language: {{hdl_language}} (default: SystemVerilog-2017). No vendor-specific pragmas.\n2) Coding style: use `logic` (SV) or `wire/reg` (Verilog) appropriately; non-blocking for sequential, blocking for combinational; no latches.\n3) Hierarchy: implement a 64:1 mux as a balanced tree using reusable leaf modules (e.g., 2:1 or 4:1 or 8:1). Depth should be {{tree_levels}}.\n4) Parameterization: make data width configurable via parameter {{param_data_width}} (default: 1); optionally allow fan-in (e.g., 2/4/8) via {{param_fanin}}.\n5) Interface: top module name {{top_module_name}} with ports:\n   - input  logic [{{data_width_minus_one}}:0] in[{{num_inputs_minus_one}}:0]   // 64 inputs of {{param_data_width}} bits each\n   - input  logic [{{sel_width_minus_one}}:0] sel   // selects one of 64 inputs\n   - output logic [{{data_width_minus_one}}:0] y\n6) Synthesis: fully combinational; avoid `casez` with don't-cares if it risks synthesis mismatch; no delays, no initial blocks.\n7) Reset/clock: NOT required (purely combinational).\n8) Include brief comments and a top-level overview banner.\n9) Emit outputs wrapped in clear tags described in `output_format`.\n10) Keep lines <= {{max_line_len}} characters; consistent indentation {{indent_spaces}} spaces.\n"
        },
        {
        "role": "user",
        "content": "[BEGIN_TASK]\nGoal: Generate a hierarchical {{hdl_language}} implementation of a {{num_inputs}}:1 multiplexer using a balanced tree of {{leaf_mux_size}}:1 leaf MUX modules.\n\nHard Constraints:\n- Top module: {{top_module_name}}\n- Leaf module: {{leaf_module_name}} (parametric on DATA_W)\n- Parameters:\n  - {{param_data_width}} = {{param_data_width_value}}   // data-path width\n  - {{param_fanin}}      = {{leaf_mux_size}}            // leaf mux fan-in (2 or 4 or 8)\n- Inputs:\n  - in[0 .. {{num_inputs_minus_one}}] : {{param_data_width_value}}-bit each\n  - sel[{{sel_width_minus_one}}:0] selects one of {{num_inputs}} inputs\n- Output:\n  - y : {{param_data_width_value}}-bit\n\nStructure Requirements:\n- Build a tree from inputs to output using instances of {{leaf_module_name}}.\n- Ensure the tree is balanced; fan-in per stage equals {{leaf_mux_size}}.\n- Number of stages = {{tree_levels}} (since {{leaf_mux_size}}^{{tree_levels}} == {{num_inputs}}).\n- Provide generate-for loops to scale across widths and bundles.\n\nQuality Bar:\n- Lint-clean with Verilator/default rules.\n- No inferred latches; purely combinational.\n- Portable across mainstream tools (Synopsys DC, Cadence Genus, Yosys).\n\nOptional (if {{emit_tb}} == true):\n- Emit a self-checking testbench {{tb_module_name}} that:\n  - Randomizes inputs and sel for {{tb_num_vectors}} trials\n  - Compares DUT output to a behavioral model\n  - Seeds RNG with {{tb_seed}}\n  - Prints PASS/FAIL; non-interactive\n\nDeliverables (wrap with tags from output_format):\n- Leaf MUX module\n- Top-level hierarchical 64:1 MUX module\n- Optional testbench\n[END_TASK]\n\nPlease produce only the tagged artifacts, nothing else.\n"
        }
    ],
    "placeholders": {
        "hdl_language": "SystemVerilog",
        "top_module_name": "mux64_tree",
        "leaf_module_name": "mux_leaf",
        "param_data_width": "DATA_W",
        "param_data_width_value": 1,
        "param_fanin": "FANIN",
        "leaf_mux_size": 4,
        "num_inputs": 64,
        "num_inputs_minus_one": 63,
        "sel_width_minus_one": 5,
        "tree_levels": 3,
        "data_width_minus_one": 0,
        "max_line_len": 100,
        "indent_spaces": 2,
        "emit_tb": true,
        "tb_module_name": "tb_mux64_tree",
        "tb_num_vectors": 200,
        "tb_seed": 42,
        "leaf_mux_size_minus_one": 3
    },
    "output_format": {
        "rtl_leaf_begin": "[BEGIN_RTL_LEAF]",
        "rtl_leaf_end": "[END_RTL_LEAF]",
        "rtl_top_begin": "[BEGIN_RTL_TOP]",
        "rtl_top_end": "[END_RTL_TOP]",
        "tb_begin": "[BEGIN_TB]",
        "tb_end": "[END_TB]"
    },
    "checks": [
        "No clocks/resets in combinational modules.",
        "No timing delays (#) or initial blocks in RTL.",
        "No x/z literals in synthesizable logic paths.",
        "Width of sel equals ceil(log2({{num_inputs}})).",
        "Leaf fan-in ^ tree_levels equals num_inputs.",
        "All generate blocks have named labels."
    ],
    "rubric": {
        "correctness": [
        "Top output equals in[sel] for all sel in [0, {{num_inputs_minus_one}}].",
        "Parameterization works for {{param_data_width}} > 1.",
        "Hierarchy uses only {{leaf_mux_size}}:1 leaves."
        ],
        "style": [
        "Clear banner comment with parameters and ports.",
        "Consistent indentation and line length.",
        "Meaningful instance and signal names."
        ]
    },
    "example_fill": {
        "explanation": "Example of expected *shape* of outputs; this is a schematic stub, not a full implementation.",
        "rtl_leaf_skeleton": "[BEGIN_RTL_LEAF]\n// {{leaf_module_name}}: {{leaf_mux_size}}:1 parametric leaf mux\nmodule {{leaf_module_name}} #(\n  parameter int {{param_data_width}} = {{param_data_width_value}}\n) (\n  input  logic [{{param_data_width}}-1:0] d[{{leaf_mux_size}}-1:0],\n  input  logic [$clog2({{leaf_mux_size}})-1:0] s,\n  output logic [{{param_data_width}}-1:0] y\n);\n  always_comb begin\n    unique case (s)\n      default: y = d[0];\n      // Fill in arms 0..{{leaf_mux_size_minus_one}}\n    endcase\n  end\nendmodule\n[END_RTL_LEAF]\n",
        "rtl_top_skeleton": "[BEGIN_RTL_TOP]\n// {{top_module_name}}: hierarchical {{num_inputs}}:1 mux built from {{leaf_mux_size}}:1 leaves\nmodule {{top_module_name}} #(\n  parameter int {{param_data_width}} = {{param_data_width_value}}\n) (\n  input  logic [{{param_data_width}}-1:0] in[{{num_inputs}}-1:0],\n  input  logic [{{sel_width_minus_one}}:0] sel,\n  output logic [{{param_data_width}}-1:0] y\n);\n  // Insert generate blocks that instantiate {{leaf_module_name}} in {{tree_levels}} stages\n  // and route select bits per-stage.\nendmodule\n[END_RTL_TOP]\n",
        "tb_skeleton": "[BEGIN_TB]\n// {{tb_module_name}}: randomized self-checking testbench for {{top_module_name}}\n// Drives {{tb_num_vectors}} vectors; prints PASS/FAIL\n[END_TB]\n"
    }
    }"""
    ...
    system_prompt = (
        f"Design: {user_prompt}\n\n"
        f"Please generate above designs based on the following template:\n"
        f"{gerenal_prompt_template}\n"
        f"{general_prompt_instruction}"
        f"Respond with ONLY valid JSON for the configuration. Nothing else. You must use the keys from the template."
        f"Ensure to configure from the provided options parameter values."
    )
...
def prompt_enhancer_simple(
    ...
    *,
    ...
    system_msg = (
        "You are an expert chip architect.\n"
        "Your task is to analyze the given User Prompt "
        "to infer the hierarchical hardware design structure where all the submodule will eventually build the topmodule.\n\n"
        "For example : to build a 64 to 1 MUX, you may need to build smaller MUX modules and a controller module.\n"
        "(like, 32 to 1 MUX, 16 to 1 MUX, 8 to 1 MUX, 4 to 1, 2 to 1 etc as submodules)\n"
        "Output Format STRICTLY:\n"
        "Top module (Hier 0)\n"
        "Description: <Generate description of the entire design>\n"
        "Submodule (Hier 1): <name>\n"
    ...
    user_content = f"""User Prompt:
                    ...
                    - To generate hierarchical hardware design structure you MUST create
                    submodules logically from topmodule to the basic modules.
                    - Each submodule should be capable of building the previous higher-level module.
                    - For example, to build a 64 to 1 MUX, you may need to create the submodules like:
                    submodules = [
                         ...
                         ["mux16to1","16-to-1 multiplexer","input [3:0] sel, input [15:0] in, output reg out"],
                         ["mux32to1","32-to-1 multiplexer","input [4:0] sel, input [31:0] in, output reg out"],
                         ["mux64to1","64-to-1 multiplexer","input [5:0] sel, input [63:0] in, output reg out"],
                     ]
                    """
...
def main():
    ...
    user_prompt = "Design a 64 to 1 multiplexer Hierarchical Verlog module."
    ppa_goal = {"freq_mhz": 250, "optimize_for": ["latency", "area"]}

    aug_path = DIR_OUT / "augmented_prompt.txt"

    prompt_enhancer_simple(
        ...
        while attempt < MAX_RETRIES and not ok:
            ...
            if hit:
                gen_code = module_generator_llm(
                    ...
                    previous_generation=code_check,
                )
            else:
                gen_code = module_generator_llm(
                    module_name=mname,
            ...
            ok = True
            msg = "Syntax check skipped (assumed PASS)."

            if ok:
                final_code = gen_code
                print(f"[{mname}] syntax PASS ✅ on attempt {attempt}")
                break
            elif attempt == MAX_RETRIES and not ok:
                final_code =gen_code
                ok = True
                print(f"[{mname}] forced PASS on final attempt {attempt}.")
                break
            else:
